...
class SendEmail():
	...
	def setOpc(self, opc):
		if opc == 1 or opc == 2:
			...
			print(":[!] Esta opcion no existe, agreguela de nuevo: ")
			self.setOpc(int(input()))
	...
	def setEmailAccount(self, email_account):
		...
		if self.getOpc() == 1:
			...
			for character in cant_be:
				...
				if character in email_account or " " in email_account:
					...
					self.setEmailAccount(input(":::: Write the email again: "))
					print()
		...
		if self.email_account == "":
			...
			self.setEmailAccount(input("[!] Write this one again: "))
	...
	def setPassword(self, password):
		...
		if self.password == "":
			...
			self.setPassword(getpass.getpass(":[!] Write this one again: "))
	...
	def setTo(self, to):
		if " " not in to:
			# Now we use a regular expression to be sure
			# our user used '@' and the domain of gmail 
			type_email = re.search(r"(.*?)(@gmail.com|@hotmail.com|@outlook.com|@outlook.es|@uanl.edu.mx|@gmail.com|@amer.teams.ms)", to)

			# Verify the regular expression
			if type_email:
				# If the regular expression found a valid
				# email, we saved it in 'self.email_account'
				self.to = type_email[0]

			else:
				to = ""
				print(":[!] You added an invalid email")
				self.setTo(input(":::: Write the email again: "))
				print()
	...
	######################################################################
	def setPictures(self, opc):
		pictures = []

		if opc == 1:
			print("::[+] Add your name image: ")
			image = input("::::: ")
			print()
			...
			if verify_image:
				...
				print(":::[+] Add the directory of your picture: ")
				directory = input(":::::: ").lower()
				print()

				if "c:\\" in directory:
					pictures.append(verify_image[0])
					pictures.append(directory)
					self.pictures.append(pictures)
	...
	def sendEmail(self, subject, msg):
		...
		if len(self.pictures) > 0:
			...
			for picture in self.pictures:
				try:
					if picture[1] == os.getcwd().lower():
						os.chdir("c:\\")
					os.chdir(picture[1])

					file = open(picture[0], 'rb')
					f_name = file.name
					f_data = file.read()
					...
					print(":::: The message will sended without the image(s).")
